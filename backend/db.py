...
def create_order(retailer_id, total_amount, order_status="Pending"):
    if not mysql_configured():
        return {
            "orderId": 9999,
            "retailerId": retailer_id,
            "retailerName": next(
                (retailer["retailerName"] for retailer in retailers_from_sample() if retailer["retailerId"] == retailer_id),
                "Sample Retailer",
            ),
    ...
    order_id = execute_write(
        """
        INSERT INTO orders (RetailerID, OrderDate, Status, TotalAmount)
        VALUES (%s, CURDATE(), %s, %s)
        """,
        (retailer_id, order_status, total_amount),
    )
...
def update_inventory(item_id, quantity=None, warehouse=None, low_stock_threshold=None):
    ...
    execute_write("UPDATE inventoryitem SET Quantity = %s WHERE ItemID = %s", (quantity, item_id))
    return fetch_one(
        """
        SELECT
            i.ItemID AS itemId,
...
def create_shipment(source, destination, shipment_status="In Transit"):
    ...
    if route:
        ...
        route_id = execute_write(
            "INSERT INTO route (Source, Destination, Distance) VALUES (%s, %s, %s)",
    ...
    shipment_id = execute_write(
        """
        INSERT INTO shipment (VehicleID, RouteID, ShipmentDate, Status)
        VALUES (%s, %s, CURDATE(), %s)
        """,
        (vehicle["vehicleId"], route_id, shipment_status),
    )
    return fetch_one(
        """
        SELECT
...
def create_payment(order_id, amount, status="Paid"):
    ...
    invoice_id = execute_write(
        """
        INSERT INTO invoice (OrderID, Amount, IssueDate)
        VALUES (%s, %s, CURDATE())
        """,
        (order_id, amount),
    )
    payment_id = execute_write(
        """
        INSERT INTO payment (InvoiceID, PaymentDate, Amount, Status)
        VALUES (%s, CURDATE(), %s, %s)
        """,
